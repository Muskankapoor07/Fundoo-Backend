...
public class CollaboratorController {
    ...
    @DeleteMapping("/api/notes/{noteId}/collaborators")
    public ResponseEntity<ApiResponse> removeCollaboratorParam(@PathVariable Long noteId, @RequestParam String email) {
    ...
    public ResponseEntity<ApiResponse> removeCollaboratorPath(@PathVariable Long noteId, @PathVariable String email) {
        String message = collaboratorService.removeCollaborator(noteId, email, getEmail());
        return ResponseEntity.ok(new ApiResponse(200, message));
    }
